...
for i = 1 : nbr_trials
    ...
    predictions_test_tree = zeros(1, nbr_test_examples);
    for j = 1 : nbr_test_examples
        predictions_test_tree(j) = predict(tree_model, X_test(:,j)');        
    end
    predictions_test_svm = zeros(1, nbr_test_examples);
    for j = 1 : nbr_test_examples
        predictions_test_svm(j) = predict(svm_model, X_test(:,j)');        
    end
    predictions_test_nn = zeros(1, nbr_test_examples);
    for j = 1 : nbr_test_examples
        predictions_test_nn(j) = predict(nn_model, X_test(:,j)');        
    end
...
%     predictions_test_nn   = predict(nn_model,X_test');
    ...
    predictions_train_tree = zeros(1, nbr_train_examples);
    for j = 1 : nbr_train_examples
        predictions_train_tree(j) = predict(tree_model, X_train(:,j)');        
    end
    predictions_train_svm = zeros(1, nbr_train_examples);
    for j = 1 : nbr_train_examples
        predictions_train_svm(j) = predict(svm_model, X_train(:,j)');        
    end
    predictions_train_nn = zeros(1, nbr_train_examples);
    for j = 1 : nbr_train_examples
        predictions_train_nn(j) = predict(nn_model, X_train(:,j)');        
    end
    % We can now proceed to computing the respective error rates.
...
%X_reshape = reshape(X,[],19);
X_reshape = reshape(X_test,19,19,1,[]);
% figure;
% colormap(gray);
% imagesc(X_reshape(:,:,1,1));
% title('First img');
% figure;
% colormap(gray);
% imagesc(X_reshape(:,:,1,3));
% title('Second img');
...
for i = 1:length(Y_test)
    if(Y_test(:,i)==1)
        ...
        %disp(["Face img - prediction - my knn: ",classify(X_test(:, i), classification_data)])
        break;
    end
end
for i = 1:length(Y_test)
    if (Y_test(:,i)==-1)
        ...
        %disp(["Second img - prediction - my knn: ",classify(X_test(:, i), classification_data)])
        break;
    end
end     

disp(["First img - prediction - my knn: ",classify(X_test(:, 1), classification_data)])
disp(["Second img - prediction - my knn: ",classify(X_test(:, 3), classification_data)])

disp(["First img - prediction - tree: ",predictions_test_tree(1)])
disp(["First img - prediction - svm: ",predictions_test_svm(1)])
disp(["First img - prediction - knn (built-in): ",predictions_test_nn(1)])

disp(["Second img - prediction - tree: ",predictions_test_tree(3)])
disp(["Second img - prediction - svm: ",predictions_test_svm(3)])
disp(["Second img - prediction - knn: ",predictions_test_nn(3)])